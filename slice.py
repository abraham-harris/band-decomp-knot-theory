...
class SliceEnv():
    ...
    def __init__(self, braid_word=[], max_braid_index=12, max_braid_length=20, inaction_penalty=0.005, starting_knot_strand=1):
        self.max_braid_length  =max_braid_length     # Maximum length of a braid word, which is fixed once the environment is instantiated
        assert len(braid_word) <= max_braid_length, "Cannot initialize with braid with length longer than max_braid_length"
        self.word = np.array(braid_word)             # The numpy array that tracks the braid word representing the knot
        self.bonus = 0                               # This is the bonus that is given to the score whenever an unlinked component is created
        self.inaction_penalty = inaction_penalty     # Penalty given for any action which results in a reward of 0 (should be a positive value)
        self.index = max_braid_index                 # Max number of strands that can be used in the braid at any given time (extra strands will be added as unlinked strands on the knot component are removed)
        self.extra_strands = 0                       # Counter used to create new labels for components that are introduced when new strands are added (following the removal of unlinked strands on the knot component)
        self.n_comp = 1                              # I'm not sure what this is, it doesn't seem to show up anywhere else.
        self.reward = 0                              # I'm also not sure what this is, self.reward also doesn't seem to show up anywhere else.
        # This list should have one entry for each strand in the braid word (self.index number of them), and tracks which strands are on the same component.
        ...
        self.components[starting_knot_strand-1] = self.component_count # This assigns the component of starting_knot_strand the number 1
        self.temp_position = len(self.word)
        # Starting with the starting_knot_strand, we trace it back through the braid word to see what other strands it connects to
